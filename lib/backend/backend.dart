...
import 'schema/speech_to_text_notes_record.dart';
...
export 'schema/speech_to_text_notes_record.dart';
...
/// Functions to query SpeechToTextNotesRecords (as a Stream and as a Future).
Future<int> querySpeechToTextNotesRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
...
Stream<List<SpeechToTextNotesRecord>> querySpeechToTextNotesRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      SpeechToTextNotesRecord.collection,
      SpeechToTextNotesRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<SpeechToTextNotesRecord>> querySpeechToTextNotesRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future<FFFirestorePage<SpeechToTextNotesRecord>>
    querySpeechToTextNotesRecordPage({
  Query Function(Query)? queryBuilder,
  DocumentSnapshot? nextPageMarker,
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, SpeechToTextNotesRecord>
      controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
        queryCollectionPage(
          ...
          if (isStream) {
            final streamSubscription =
                (page.dataStream)?.listen((List<SpeechToTextNotesRecord> data) {
              for (var item in data) {
                final itemIndexes = controller.itemList!
                    .asMap()
                    .map((k, v) => MapEntry(v.reference.id, k));
